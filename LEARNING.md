...
- **原因：** v7 34 条实测分层（v6：全局目标优先 + 安全检查回退）从纯网络 32.4%/29.4% 碰撞提升到 **82.4% 到达/0% 碰撞**，6/8 场景 100%；成功样本位姿误差小。但 S3 紧 bay（纯网络本身不安全，不可回退，局部规划无法入位）与 S7 平行（目标贴墙，局部规划到目标失败、纯网络轨迹贴墙但实际安全）需求相反，运行时策略补丁到极限，需训练侧解决。
