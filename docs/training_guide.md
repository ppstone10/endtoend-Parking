...
**v9 闭环诊断结论（2026-08-31，见 `docs/closed_loop_diagnosis.md`）**：开环与净空全部通过，但纯网络闭环成功率仅 2.9%~17.6%，远低于 70% 准入线。v7 成功 11 条在全部安全微调模型下一致失败，证明根因在"安全微调路径本身"而非净空权重（w=0/0.25/0.5/1.0）或 TF 课程。逐重规划对比定位：车辆距目标 <3m 时安全模型停止长度预测提前触发（轨迹骤缩）、终点距目标与航向误差膨胀、含方向切换，MPC 追着短而偏航的参考轨迹摆动 → 振荡。恢复数据（258 条/9.7%）是加重因子而非根因（方向 A：用纯 v7 数据微调后门禁成功率升至 47.1% 但仍摧毁 v7 能力）。

因此新增近端终点对齐监督（`SAFE-LOSS-003`，`endpoint_alignment_weight`，默认 0 关闭）对抗净空损失诱导的"缩短轨迹躲障碍"倾向，并把恢复数据采集改为用安全模型自身 deployment。后续调优保持单变量：只比较 `endpoint_alignment_weight=0.25/0.5`（同时固定其余配置），看纯网络闭环成功率是否回升；不得同时改恢复数据占比、净空权重或控制参数。恢复数据重采必须用本轮模型自身的 deployment checkpoint，不能沿用 v7 deployment 的旧恢复集。

**v10/v11 实测（2026-08-31，已定论）**：近端对齐监督（goal/target 两种形式，w=0.5）均使纯网络闭环更差（0%~2.9% 成功、52.9%~73.5% 碰撞）；净空目标豁免（1m 半径完全豁免，`safety_goal_exempt_radius_m`/`safety_goal_exempt_weight`）也不改善（0% 成功/47.1% 碰撞）。失败样本在 v9/v9d/v10/v11 间高度一致。结论：闭环失败不是"净空约束过强"或"缺近端监督"单独导致，而是安全微调整体（净空+恢复数据+微调组合）改变了网络滚动重规划行为。净空豁免与近端对齐作为能力保留（默认关闭），不再作为正式训练的默认配置。
...
- 判定：分层目标是"保留纯网络能力 + 用局部规划消除碰撞/近端退化"。按第一标准验收：到达+位姿达标、碰撞=0（硬约束）。若分层到达率低于纯网络（如 S7 平行），应回退纯网络。
- 实测 v7 34 条（v6）：到达率 82.4%（28/34）、碰撞 0%（硬约束满足）、6/8 场景 100%；剩余 S3 紧 bay ×5 + S7 平行 ×1 未到达。
