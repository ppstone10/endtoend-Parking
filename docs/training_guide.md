...
1. `scripts/build_recovery_dataset.py` 在原 train 任务中运行当前 deployment，按固定步长、碰撞前最后安全状态、位置偏离 ≥0.25m 或航向偏离 ≥5° 选取学习器实际访问状态；专家从该状态重新规划到原 selected goal。默认 240 个分层源任务、每任务最多 2 条，最多约占原 2400 条训练集的 20%。
...
恢复采集目录的 `.checkpoints/identity.json` 绑定数据文件、计划指纹、deployment SHA-256、车辆模型和全部选择参数；每个源任务有独立完成标记，原命令可续建，身份变化必须换输出目录。`report.json` 记录场景、任务、机动、触发类型、规划失败原因和最终合并归档。原始 train 与 v7 checkpoint 都不会被覆盖。
...
调优时保持单变量：若纯网络 val 碰撞率仍高且开环轨迹指标未退化，先把 `collision_loss_weight` 从 0.5 提到 1.0；若 ADE/FDE 明显恶化而 collision loss 已接近 0，则降到 0.25。若干预集中在未覆盖的偏离状态，先扩大恢复源任务数，不同时改模型容量。不得用带专家回退的成功率替代纯网络能力；安全门禁的干预率越高，说明网络本身仍越不安全。
